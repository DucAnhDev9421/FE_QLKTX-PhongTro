...
import { assetService, RoomAssetRequest } from '../../../services/asset';
import Alert from '../../../components/ui/Alert';

interface Props {
    roomId: number;
    roomAssetToEdit: any | null;
    onClose: () => void;
}

export default function RoomAssetModal({ roomId, roomAssetToEdit, onClose }: Props) {
    const queryClient = useQueryClient();
    const isEdit = !!roomAssetToEdit;

    const [assetId, setAssetId] = useState<number | ''>(
        roomAssetToEdit ? roomAssetToEdit.assetId : ''
    );
    const [quantity, setQuantity] = useState<number | ''>(
        roomAssetToEdit ? roomAssetToEdit.quantity : 1
    );
    const [conditionStatus, setConditionStatus] = useState(
        roomAssetToEdit ? roomAssetToEdit.conditionStatus : 'GOOD'
    );
    ...
    const mutation = useMutation({
        mutationFn: async (payload: RoomAssetRequest) => {
            if (isEdit) {
                return assetService.updateRoomAsset(roomAssetToEdit.roomAssetId, payload);
            } else {
                return assetService.assignAssetToRoom(payload);
            }
        },
        onSuccess: () => {
            ...
            >
                <div className="flex items-center justify-between p-5 border-b border-slate-800 shrink-0">
                    <h2 className="text-xl font-bold text-white flex items-center gap-2">
                        {isEdit ? 'Cập nhật Tài sản Phòng' : 'Gán Tài sản vào Phòng'}
                    </h2>
                    <button 
                        onClick={onClose}
                ...
                <div className="p-5 overflow-y-auto">
                    ...
                    <form id="room-asset-form" onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            <select 
                                ...
                                disabled={isEdit || mutation.isPending || isLoadingAssets}
                            >
                                <option value="">-- Chọn danh mục tài sản --</option>
                                {isEdit ? (
                                    <option value={roomAssetToEdit.assetId}>{roomAssetToEdit.assetName}</option>
