...
export default function TerminateModal({ isOpen, onClose, contractData }: Props) {
    ...
    const handleSubmit = (e: FormEvent) => {
        ...
        if (deduction > deposit) {
            setErrorMsg('Tiền khấu trừ không thể lớn hơn tiền cọc.');
            return;
        }
    ...
    return createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 font-sans bg-rose-950/20 backdrop-blur-md">
            <div 
                className="bg-slate-900 border border-rose-900/50 w-full max-w-md rounded-2xl shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200 overflow-hidden"
                onClick={e => e.stopPropagation()}
            >
                {/* Header */}
                <div className="flex items-center justify-between p-5 border-b border-rose-900/30 bg-rose-950/10">
                    <div className="flex items-center gap-3">
                        <div className="p-2 bg-rose-500/20 rounded-lg text-rose-500">
                            <AlertTriangle size={20} />
                        </div>
                        <h2 className="text-xl font-bold text-rose-100">Thanh lý Hợp đồng</h2>
                    </div>
                    <button 
                        disabled={mutation.isPending}
                        onClick={onClose}
                        className="p-2 text-rose-400 hover:text-rose-100 rounded-lg hover:bg-rose-900/50 transition-colors"
                    >
                        <X size={20} />
                    </button>
                </div>
                ...
                <div className="p-5">
                    ...
                    <form id="terminate-form" onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-sm font-medium text-slate-400 mb-1">Tiền khấu trừ từ cọc (VNĐ)</label>
                            <input 
                                type="number"
                                className="w-full bg-slate-950 border border-slate-700 text-white px-4 py-2.5 rounded-xl focus:outline-none focus:ring-1 focus:ring-rose-500 hover:border-rose-500/50 transition-colors font-mono"
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-400 mb-1">Lý do khấu trừ (nếu có)</label>
                            <textarea 
                                className="w-full bg-slate-950 border border-slate-700 text-white px-4 py-2.5 rounded-xl focus:outline-none focus:ring-1 focus:ring-rose-500 hover:border-rose-500/50 transition-colors min-h-[80px]"
                        ...
                        <div className="flex justify-between items-center p-4 bg-slate-950 rounded-xl border border-slate-800">
                            <span className="text-slate-400 text-sm">Hoàn trả khách thuê:</span>
                            <span className={`text-lg font-bold ${refund > 0 ? 'text-emerald-500' : 'text-slate-500'}`}>
                                {new Intl.NumberFormat('vi-VN').format(refund)}đ
                            </span>
                        </div>
                    </form>
                </div>
